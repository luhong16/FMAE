...
results_dict = np.load(f'{data_dir}/FMAE_fig3a_cell_capacity.npy', allow_pickle=True).item()

print(results_dict)

vit_pretrained_ae_from_dict = np.concatenate(results_dict['Cell']['FMAE']['AE_list'])
LSTM_ae_from_dict = np.concatenate(results_dict['Cell']['LSTM']['AE_list'])
RF_ae_from_dict = np.concatenate(results_dict['Cell']['RF']['AE_list'])
XGBoost_ae_from_dict = np.concatenate(results_dict['Cell']['XGBoost']['AE_list'])
patchtst_ae_from_dict = np.concatenate(results_dict['Cell']['PatchTST']['AE_list'])
itransformer_ae_from_dict = np.concatenate(results_dict['Cell']['iTransformer']['AE_list'])
timemixer_ae_from_dict = np.concatenate(results_dict['Cell']['TimeMixer++']['AE_list'])

# withour numbers
fig, ax = plt.subplots(figsize = (10,8))

visualize_orders = ['FMAE', 'LSTM', 'XGBoost', 'RF', 'PatchTST', 'iTransformer', 'TimeMixer++']
data = [list(np.concatenate(results_dict['Cell'][order]['AE_list'])) for order in visualize_orders]
colors = [results_dict['Cell'][order]['color'] for order in visualize_orders]
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, f'FMAE_fig3a_cell_capacity.png'))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, f'FMAE_fig3a_cell_capacity_w_numbers.png'))
plt.close()