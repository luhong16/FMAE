...
import sys
sys.path.append('./baselines/Time_Series_Library')
sys.path.append('./baselines/util')
...
def main(args):
    ...
    if args.model.startswith("LSTM"):
        ...
        from Time_Series_Library.models import PatchTST
        ...
        configs = {
            ...
            'activation': 'relu',
            'e_layers': 6,
            'enc_in': len(data_task.encoder)
        }
        model = PatchTST.Model(configs =  SimpleNamespace(**configs), patch_len = 16, stride = 16)
    elif args.model.startswith('iTransformer'):
        from Time_Series_Library.models import iTransformer
        from types import SimpleNamespace
        configs = {
            'task_name': 'short_term_forecast',
            'seq_len': 128,
        ...
        model = iTransformer.Model(configs = SimpleNamespace(**configs))
    else:
    ...
    if args.model.startswith("LSTM") \
        or args.model.startswith('PatchTST') \
            or args.model.startswith('iTransformer'):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.downstream in ['anomaly']:
            ...
            if args.downstream in ['RUL']:
                ...
                valid_cell_level_rmse, valid_cell_level_percentage_error = evaluate_RUL(valid_res, args.inference_cycle_id)
