...
def set_args():
    parser = argparse.ArgumentParser(description='ElasticLinear_discharge')
...
def main():
    ...
    if args.Predicting_log10: 
        ...
        print(f"Evaluating RUL_w_log baseline discharge brand_{args.Brand} fold_{args.Fold} points_{args.Points} ...")
    else:
        results_path = f"./baselines/results/RUL_wo_log"
        scores_path = f"./baselines/results/RUL_wo_log/diff_models_losses"
        print(f"Evaluating RUL_wo_log baseline discharge brand_{args.Brand} fold_{args.Fold} points_{args.Points} ...")
    ...
    for l1 in l1_lambdas:
        for l2 in l2_lambdas:
            for lr in lrs:
                models.append(f"ElasticLinear_discharge_l1_{l1}_l2_{l2}_lr_{lr}")
        ...
        for l1 in l1_lambdas:
            for l2 in l2_lambdas:
                for lr in lrs:
                    ...
                    output = f"{args.Task}_brand{args.Brand}_fold{args.Fold}_points{args.Points}_ElasticLinear_discharge_l1_{l1}_l2_{l2}_lr_{lr} | " + best_records
                    print(output)
                    losses.append(vali_loss)
                    y_preds.append(y_pred)
                    GTs.append(GT)
                    y_trains.append(y_train)
        torch.save((models, losses, GTs, y_preds, y_train), os.path.join(scores_path, f'Brand{args.Brand}_Fold{args.Fold}_points{args.Points}_ElasticLinear_discharge_wo_log_vali_losses.pkl'))
        print(models, len(models))
        ...
        for l1 in l1_lambdas:
            for l2 in l2_lambdas:
                for lr in lrs:
                    ...
                    GTs.append(GT)
                    y_trains.append(y_train)
        torch.save((models, losses, GTs, y_preds, y_trains), os.path.join(scores_path, f'Brand{args.Brand}_Fold{args.Fold}_points{args.Points}_ElasticLinear_discharge_w_log_vali_losses.pkl'))
        print(models, len(models))
        print(losses, len(losses))
        print([(GTs[idx], y_preds[idx]) for idx in range(len(GTs))])
