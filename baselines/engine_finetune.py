...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        elif args.mask_type in ["except_current"]:
            if args.task == "batterybrandmileage":
                mask_channel = torch.tensor([1,0,1,1,1,1,1,1,1], dtype=torch.float32, device=device).unsqueeze(-1).repeat(B, 1, 1)
            else:
                mask_channel = torch.tensor([1,0,1,1,1,1,1,1], dtype=torch.float32, device=device).unsqueeze(-1).repeat(B, 1, 1)
        elif args.mask_type in ["except_soc"]:
            if args.task == "batterybrandmileage":
                mask_channel = torch.tensor([1,1,0,1,1,1,1,1,1], dtype=torch.float32, device=device).unsqueeze(-1).repeat(B, 1, 1)
            else:
                mask_channel = torch.tensor([1,1,0,1,1,1,1,1], dtype=torch.float32, device=device).unsqueeze(-1).repeat(B, 1, 1)
        else:
            mask_channel = None
    ...
    for batch in metric_logger.log_every(data_loader, 200, header):
        ...
        if args.mask_type in ["max_min_volt_temp", "use_volt_current_soc"]:
            ...
            if args.task == "batterybrandmileage":
                ...
                mask_channel = torch.tensor([0,0,0,0,1,1,1,0], dtype=torch.float32, device=device).unsqueeze(-1).repeat(B, 1, 1)
        elif args.mask_type in ["max_min_volt_temp_cycle"]:
            if args.task == "batterybrandmileage":
                mask_channel = torch.tensor([0,0,0,1,1,1,1,0,1], dtype=torch.float32, device=device).unsqueeze(-1).repeat(B, 1, 1)
            else:
