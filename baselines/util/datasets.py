...
def load_dataset(fold_num, brand_num, same_normalizer=False, car_dict_dir='five_fold_utils_six_brand_all', downstream='', data_type=None, normalizer = None, dataset_fn=None, ind_ood_car_dict=None, all_car_dict=None, num_snippet=0, cycle_gap=0, cycle_id=None, data_percent=100, seed=0, normalizer_lab=None, task='', apply_noise = False):
    ...
    for (___, each_num) in enumerate(car_number):
        ...
        for pkl_all in pkls:
            ...
            if apply_noise:
                # add guassian noise with 0.005 variance on x[0], which has length 128
