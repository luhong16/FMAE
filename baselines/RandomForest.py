...
def set_args():
    ...
    parser.add_argument('--Task', type = str, default = 'capacity')
...
def main():
    ...
    # RF
    num_trees = [50] # searched num_trees = [6, 25, 50, 100, 250, 500, 1000]
    tree_depths = [11] # searched tree_depths = [3, 6, 7, 9, 11]
    # GPU
    ...
    for tree in num_trees:
        for depth in tree_depths:
            models.append(f"RF_t_{tree}_d_{depth}_train_{args.percent}")
    ...
    for tree in num_trees:
        for depth in tree_depths:
            net = RFRegressor(n_estimators = tree, max_depth = depth, random_state = seed, n_jobs = -1).fit(X_train, y_train)
            train_loss, vali_loss= RMSE(net.predict(X_train), y_train), RMSE(net.predict(X_vali), y_vali)
            output = f"{args.Task}_brand{args.Brand}_fold{args.Fold}_RF_trees{tree}_depth{depth}_train_{args.percent} | train_loss: {train_loss} | vali_loss: {vali_loss}"
            print(output)
            losses.append(vali_loss)
            y_preds.append(net.predict(X_vali))
            # print(net.predict(X_vali))
    torch.save((models, losses, y_vali, y_preds), os.path.join(scores_path, f'Brand{args.Brand}_Fold{args.Fold}_train_{args.percent}_RF_vali_losses.pkl'))
    print(models, len(models))
