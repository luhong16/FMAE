...
from util.datasets import load_dataset, Normalizer, PreprocessNormalizer
from util import tasks

def get_args_parser():
    parser = argparse.ArgumentParser('FMAE pre-training', add_help=False)
    ...
    parser.add_argument("--wandb-project", type=str, default="battery")
    parser.add_argument("--wandb-group", type=str, default="mae_pretrain")
    parser.add_argument("--job-type", type=str, default="training-")
    parser.add_argument("--wandb-name", type=str, default="")
    parser.add_argument("--user-name", type=str, default="luhong160")
    ...
    parser.add_argument('--mask_patch_ratio', default=0.75, type=float,
                        help='Masking patch ratio (percentage of removed patches).')
    parser.add_argument('--mask_channel_ratio', default=0, type=float,
                        help='Masking channel ratio.')
    parser.add_argument('--mask_snippet_num', default=0, type=int,
                        help='Masking snippet num.')
    ...
    parser.add_argument("--decoder_pos_embed_dim", default=-1, type=int) 
    parser.add_argument("--decoder_type", default='combine', type=str) 
    parser.add_argument("--decoder_pad_type", default='mask_token', type=str) 
...
def main(args):
    ...
    if args.decoder_pos_embed_dim != -1:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.num_snippet > 0:
            data_loader_train.dataset.shuffle_pretrain()
...
if __name__ == '__main__':
    ...
    args.output_dir += 'mae_pretrain' + args.model + \
                        '_mpr' + str(args.mask_patch_ratio) + \
                        '_mcr' + str(args.mask_channel_ratio) + \
                        '_msn' + str(args.mask_snippet_num) + \
                        '_lr' + str(args.lr) + \
                        '_blr' + str(args.blr) + \
                        '_minlr' + str(args.min_lr) + \
                        '_weightdecay' + str(args.weight_decay) + \
                        '_warmupepoch' + str(args.warmup_epochs) + \
                        '_numsnippet' + str(args.num_snippet) + \
                        '_pos_embed_dim' + str(args.pos_embed_dim) + \
                        '_d_pos_embed_dim' + str(args.decoder_pos_embed_dim) + \
